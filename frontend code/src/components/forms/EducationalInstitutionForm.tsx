...
const EducationalInstitutionForm = ({
  ...
  const { isLoading: fetchDataLoader, data: EducationalInstitutionDetails } =
    useQuery<any, any>({
  ...
  useEffect(() => {
    if (EducationalInstitutionDetails?.data) {
      const data = EducationalInstitutionDetails?.data;
      form.setFieldsValue({
        ...data,
      });
    }
  ...
  const { isPending, mutate: createOrEditMutate } = useMutation({
    mutationFn: (EducationInstituationData) => {
      if (isEditMode) {
        return axiosPrivate.post(
          EndPoints.UPDATE_EDUCATIONAL_INSTITUTION_PROFILE,
          EducationInstituationData
        );
      }
      return isAdminFlow
        ? axiosPrivate.post(
        ...
        : API.post(EndPoints.USER_REGISTRATION, EducationInstituationData);
    },
    onError: (err: any) => {
      console.log("err ", err);
    ...
    onSuccess: (res, variables: any) => {
      setNewUserDetails({
        userType: UserRole.EDUCATIONAL_INSTITUTION,
  ...
  const onFinish = (values: any) => {
    ...
    if (isEditMode) {
      const educationalUpdateData: any = {
        ...values,
        dateOfBirth: dayjs(values.dateOfBirth).unix(),
      };
      delete educationalUpdateData.email;
      createOrEditMutate(educationalUpdateData);
    } else {
      const EducationalInstitutionRegData = {
        ...values,
        userType: UserRole.EDUCATIONAL_INSTITUTION,
      };
      createOrEditMutate(EducationalInstitutionRegData);
    }
  };
